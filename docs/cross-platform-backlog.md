...
## График отставания от лидера / race graph (перенос из Android)

**Статус**: не реализовано. Реализовано в `competra-android` (`RaceGraphScreen` в `:feature:center` + `EventRaceGraphScreen` в `:core:eventdetails`, экраны-двойники по тому же паттерну, что и таблица сплитов выше).

**Что нужно**: отдельный экран — визуальное сравнение участников группы: отставание от лидера в секундах на каждом КП (аналог графика WinSplits). X — КП по порядку дистанции, Y — отставание (лидер всегда на нуле). Точка входа — кнопка «График» рядом с кнопкой «Сплиты» на экране результатов группы.

### Расчёт (переиспользовать `buildSplitsTable`, не дублировать anchor/cumulative-логику)

На Android это не отдельный алгоритм, а тонкая надстройка над уже существующей `SplitsTable` (см. раздел про таблицу сплитов выше): `buildRaceGraphData(table: SplitsTable)` в том же файле `SplitsTableBuilder.kt` —
- по каждой колонке (КП) берётся минимальное `cumulativeSeconds` среди участников со статусом `FINISHED` — это лидер;
- для каждого финишировавшего участника на каждом КП дельта = `cumulativeSeconds(участник) - cumulativeSeconds(лидер)`;
- участники со статусом отличным от `FINISHED` (`DNS`/`DNF`/`DSQ`/`REGISTERED`/`STARTED`) исключаются целиком — их кривая отставания не имеет смысла (согласовано отдельно: DSQ исключается наравне с DNS/DNF, хотя формально КП мог быть пройден).

Если заводить аналог `buildSplitsTable()` в `:shared` (см. предыдущий раздел), `buildRaceGraphData()` стоит перенести туда же — она принимает на входе именно `SplitsTable`, а не сырые данные группы, так что должна жить рядом.

### UI

- На Android использован [Vico](https://github.com/patrykandpatrick/vico) (`com.patrykandpatrick.vico:compose`, чистый Compose Multiplatform, без View-зависимостей) — `CartesianChartHost` + `LineCartesianLayer`, один `Line` на участника со своим цветом. Библиотека кроссплатформенная (Compose Multiplatform), поэтому теоретически подключаема и в `:web` без замены на другой чарт.
- Легенда и переключение видимых линий совмещены в одном списке под графиком: чекбокс — добавить/убрать линию, тап по строке — подсветить линию участника (остальные линии тускнеют). Отдельного hit-testing по точкам на канвасе графика не делали — это сознательное упрощение, из-за отсутствия готового API в Vico для точного тап-по-точке без markers/API с неочевидным поведением.
- Дефолт при большой группе: по умолчанию видны только топ-10 по итоговому месту (`result.rank`), остальные скрыты — иначе график с 30+ линиями нечитаем. Список под графиком показывает всех финишировавших, включая скрытых.

### На что обратить внимание

- Экран специально сделан «двойником» (два раздельных `Screen`/`ViewModel` — один на локальных Room-данных для организатора, другой на remote-данных для публичного просмотра), а не общим компонентом с двумя источниками данных — по тому же паттерну, что и таблица сплитов. В web, где нет offline-first разделения на локальный/remote слой, скорее всего понадобится только один экран.
- Цвета линий — заданы статичной палитрой по индексу участника в `:core:ui`, не зависят от темы Android — в web их можно взять из палитры темы Compose Multiplatform Web, если такая уже используется для похожих кейсов (например, для графиков/статистики), иначе завести отдельную палитру по аналогии.
